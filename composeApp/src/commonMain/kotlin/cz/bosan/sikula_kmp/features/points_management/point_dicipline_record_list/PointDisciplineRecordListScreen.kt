...
@Composable
fun PointDisciplineRecordListRoute(
    onAddClick: (Discipline, String?, Int) -> Unit,
    onCrewRecordClick: (Discipline, Crew) -> Unit,
    onBackClick: () -> Unit,
    navigationBarActions: NavigationBarActions,
    modifier: Modifier = Modifier,
    viewModel: PointDisciplineRecordListViewModel = koinViewModel(),
) {
    val state by viewModel.state.collectAsStateWithLifecycle()

    PointDisciplineRecordListScreen(
        state = state,
        modifier = modifier,
...
@Composable
private fun PointDisciplineRecordListScreen(
    state: PointDisciplineRecordListState,
    modifier: Modifier = Modifier,
    onAction: (PointDisciplineRecordListAction) -> Unit,
    navigationBarActions: NavigationBarActions,
    onAddClick: (Discipline, String?, Int) -> Unit,
    onCrewRecordClick: (Discipline, Crew) -> Unit,
    onBackClick: () -> Unit,
    ...
    LaunchedEffect(pagerState.currentPage) {
        ...
        if (newDay != state.campDay) {
            onAction(PointDisciplineRecordListAction.OnChangeCampDay(newDay))
        }
    }
    ...
    Scaffold(
        ...
        topBar =
        {
            DisciplineTopBar(
                ...
                showInfoIcon = false,
                submitRecords = {
                    onAction(PointDisciplineRecordListAction.OnSubmitRecords)
                },
                unSubmitRecords = { }
            )
        },
        bottomBar = {
            ...
            if (state.enabledCreatingRecords == true) {
                FloatingActionButton(
                    onClick = { onAction(PointDisciplineRecordListAction.OnCrewsWithoutRecords) },
        ...
        Column(
            ...
            CampDayCarousel(
                ...
                onDayChanged = { campDay ->
                    onAction(
                        PointDisciplineRecordListAction.OnChangeCampDay(
                            campDay
                        )
                    )
                },
                                ...
                                if (state.warningMessage != null) {
                                    ...
                                    TeamRecordList(
                                        ...
                                        onRecordChange = { record, value, comment ->
                                            onAction(
                                                PointDisciplineRecordListAction.OnRecordUpdate(
                                                    record,
                                                    value,
                                                    comment
                                                )
